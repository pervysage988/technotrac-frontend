...
import { Truck, ArrowUpRight, ArrowDownLeft, MessageSquare, PlusCircle, Loader2 } from "lucide-react";
...
// Assume the logged-in user is the first user for demo purposes
const loggedInUser = users[0];
const MAX_DISTANCE_KM = 70; // Same radius as equipment search
...
export default function LogisticsPage() {
  ...
  const fetchLogistics = useCallback(async () => {
    setIsLoading(true);
    try {
      const data = await fetchLogisticsRequests();
      setLogisticsRequests(data);
    } catch {
      toast({ variant: "destructive", title: "Error", description: "Could not load logistics requests." });
    } finally {
      setIsLoading(false);
    }
  }, [toast]);
          ...
          <CardContent className="space-y-6">
            {isLoading ? (
              ...
              nearbyRequests.map(req => {
                ...
                return (
                  <Card key={req.id} className="p-4 flex flex-col sm:flex-row items-start gap-4 hover:bg-muted/50 transition-colors">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-semibold">{requester.displayName} <span className="text-xs text-muted-foreground">from {requester.location}</span></p>
                          <p className="text-sm text-muted-foreground">Needs to <span className="font-medium">{req.type}</span>: <span className="font-semibold text-primary">{req.quantity} of {req.goods}</span></p>
                        </div>
                         <Badge variant={isExport ? "secondary" : "outline"} className="gap-1">
                          {isExport ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownLeft className="h-3 w-3" />}
                          {req.type}
                        </Badge>
                      </div>
                       <div className="border-l-2 pl-3 mt-2 text-sm">
                          <p><span className="font-semibold">Destination:</span> {req.destination}</p>
                          <p><span className="font-semibold">When:</span> {req.date}</p>
                      </div>
                    </div>
